...
import it.mc.jhmkapp.config.KafkaProperties;
...
class JhmkappKafkaResourceIT {
    ...
    private static void startTestcontainer() {
        ...
        kafkaContainer = new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:5.5.7")).withNetwork(null);
    ...
    void setup() {
        ...
        JhmkappKafkaResource kafkaResource = new JhmkappKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhmkapp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhmkapp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
